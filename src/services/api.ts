...
export default {
	...
	getProductAttachments(channelId: string, productId: string) {
		return liferay.get(
			`o/headless-commerce-delivery-catalog/v1.0/channels/${channelId}/products/${productId}/attachments?accountId=-1`,
		);
	},

	getProductImages(channelId: string, productId: string) {
		return liferay.get(
	...
	getSpecification() {
		return liferay
			...
			.json<APIResponse<ProductSpecification>>();
	},
	...
	getCatalogSearch(searchParams: URLSearchParams = new URLSearchParams()) {
		return liferay.get(
	...
	getTaxonomyVocabularies(siteId: number) {
		return liferay.get(
	...
	getCategories(vocabularyId: number) {
		return liferay
			.get(
