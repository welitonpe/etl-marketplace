...
class MigrateProductVersions {
	...
	async retry<T>(
		...
		while (attempts < retries) {
			try {
				...
				console.error(`Attempt ${attempts} failed. Retrying in ${delay}ms...`);
			...
			for (const virtualEntry of productVirtualSettingsFileEntries) {
				...
				if (!alreadyCreatedEntry.length) {
					...
					if (!virtualSettingsId) {
						...
						await this.retry(() =>
							api.updateProduct(productId, { productVirtualSettings: {} }),
						);

						const productResponse = await this.retry(() =>
					...
					await this.retry(() =>
						api.createProductVirtualEntry(virtualSettingsId, formData),
					)
						.then(() =>
							this.logger.info(
	...
	async processLPKGs(entryPath: string, product: Product) {
		const hasExtractedFolder = await this.retry(() =>
			fs.exists(path.join(entryPath, PACKAGE_FOLDER_NAME)),
		);
		...
		const lpkgs = await this.retry(() =>
			fs.readdir(path.join(entryPath, PACKAGE_FOLDER_NAME)),
		);
		...
		for (const lpkg of lpkgs) {
			...
			console.log("processLPKGs  versionName:", versionName)
			...
			const attachment = await this.retry(() =>
				fs.readFile(path.join(entryPath, PACKAGE_FOLDER_NAME, lpkg), {
					encoding: "base64",
				}),
			);
			...
			// Verifica se já existe uma especificação com o mesmo valor (versionName)
			const liferayVersions = productSpecifications.filter(
				(specification) =>
					specification.specificationKey ===
						SPECIFICATION_LIFERAY_VERSION_KEY &&
					specification.value.en_US === versionName, // Verifica se a versão já existe
					console.log("processLPKGs  versionName:", versionName)
			);

			if (!liferayVersions.length) {
				// Não existe uma especificação com o valor atual, então cria uma nova
				if (
					...
					console.log("processLPKGs  versionName:", versionName)
					await this.retry(() =>
						api.createProductSpecification(Number(productId), {
							...
							value: {
								...
								console.log("api.createProductSpecification  versionName:", versionName)
						}),
					);
				}
				...
				// Caso já exista uma especificação com o mesmo valor, não cria outra
				this.logger.info(
					`Specification ${versionName} already exists, skipping creation.`,
				);
			}
		}
			console.log("processLPKGs  versionName:", versionName)
		...
		await this.retry(() =>
			this.createVirtualItem(product, productVirtualSettingsFileEntries),
		)
