    ...
    async createVirtualItem(
        ...
        try {
            ...
            for (const virtualEntry of productVirtualSettingsFileEntries) {
                ...
                let virtualSettingsId = product?.productVirtualSettings?.id;

                const alreadyCreatedEntry =
                    productVirtualSettingsFileEntries.filter(
                        (alreadyCreatedVirtualEntry) =>
                            alreadyCreatedVirtualEntry.version ===
                            virtualEntry.version
                    );
                ...
                if (!alreadyCreatedEntry.length) {
                    ...
                    if (!virtualSettingsId) {
                        ...
                        const productResponse = await api.getProductById(
                            product.productId
                        );

                        virtualSettingsId =
                            productResponse?.productVirtualSettings?.id;
                    }
                    ...
                    await api
                        ...
                        .catch(() =>
                            this.logger.error(
                                "Unable to process virtual items individually"
                            )
                        );
                }
            }
