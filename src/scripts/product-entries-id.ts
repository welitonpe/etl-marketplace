...
const LIFERAY_VERSION_VOCABULARY = "marketplace liferay version";
...
class ProductEntryIds {
	...
	async init() {
		const file = await Bun.file(path.join(paths.csv, "appEntry.csv")).text();
			...
			if (hasLiferayVersion) {
				...
				const productCategories = product.categories.reduce(
					(specifications: string[], category) => {
						if (category.vocabulary === "marketplace liferay version") {
